...
public class LetterMatchingController {
     ...
     */
    @PostMapping("/api/random-letters/vaild")
    public ResponseEntity<BaseResponse<CheckLastMatchResponse>>checkLastMatched(){
        ...
        BaseResponse<CheckLastMatchResponse> response = BaseResponse.of(checkLastMatchResponse, "새 편지 매칭 까지 남은 시간");
        return ResponseEntity.ok(response);
    }
     ...
     */
    @PostMapping("/api/random-letters/vaild-table")
     ...
     * 매칭 취소 하기
     */
    @PostMapping("/api/random-letters/matching/cancel")
